...
(function ($) {
    ...
    var console = window.console && window.MN_DEBUG ? window.console : {
        log: function () {
        }
    }
    ...
    jQuery(document).one('sfm_doc_body_arrived', function () {
		...
		var opts = window.MN_Opts;

        document.write = function () {
            console.log('Materialize plugin debug: using document.write is bad practice and not supported')
        };
		...
		execFunc(function () {
            var LM = window.LM || (function () {
                    ...
                    function init() {
                        ...
                        var bodyClasses = 'materialize-on sfm-style-' + opts.sidebar_style + ' sfm-sidebar-' + behaviour + ' sfm-toggle-' + opts.opening_type + (opts.blur === 'blur' ? ' sfm-blur' : '');
                        console.log(bodyClasses);
                        $body.addClass(bodyClasses);

                        $( document ).ajaxComplete(function() {
                            setTimeout(function(){
                                if(!$body.is('.materialize-on')) {
                                    $body.addClass(bodyClasses);
                                }
                            }, 0);
                        });
