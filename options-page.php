<div id="mn-options-wrap" class="widefat" style="opacity:0;">
    <script>
        var colorscheme = [
            '#c0392b',
            'a3503c',
    ...
    function custom_do_settings_sections($page)
    {
        ...
        foreach ((array)$wp_settings_sections[$page] as $section) {
            ...
            echo "<div class='settings-form-row mn_label_sbmt'><input name='Submit' type='submit' id='sbmt_{$section['id']}' class='button-primary' value='Save Changes' /></div>";
    ...
    <h2 class="form-title"><img src="<?php echo plugins_url('/img/', __FILE__); ?>logo.png" alt="" target="_blank">
        Materialize <?php echo MN_VERSION_NUM ?> Settings </h2>
    <form method="post" action="options.php" enctype="multipart/form-data">
        <ul id="tabs-copy" class="section-tabs"></ul>
        <ul id="tabs" class="section-tabs"></ul>
        <?php settings_fields('mn_options'); ?>
        <?php custom_do_settings_sections('mn'); ?>
    </form>
    <div class="la">
        <span>
            Materialize v.
            <?php echo MN_VERSION_NUM; ?>
        </span>
        |
        <a target="_blank" href="http://materialize.mtaandao.com/docs/Getting_Started">
            Documentation
        </a>
        |
        <a target="_blank" href="http://codecanyon.net/user/mtaandao/portfolio">Awesome plugins</a>
    </div>
    <div id="fade-overlay">
...
<script type="text/javascript">
    ...
    (function () {
        ...
        if ($ != null) {
            ...
            $wrap = $('#mn-options-wrap')
            $tabs = $('#tabs');
            $win = $(window);

            $(function () {

                var Materialize = Backbone.Model.extend();
                window['materialize'] = new Materialize(<?php echo $js_options; ?>);
                window['materialize_view'] = new MaterializeView({
                    model: window['materialize']
                });
                window['materialize_view'].setElement('#mn-options-wrap');
                window['materialize_view'].render();
                ...
                $('#mn-options-wrap .postbox').each(function (i, el) {
                    var $t = $(this);
                    var id = $t.attr('id');
                    var txt = $t.find('h3').html();
                    var icon = '<i class="flaticon-' + map[id] + '"></i>';
                    var active = isLS && sessionStorage.getItem('mn-section') && id === sessionStorage.getItem('mn-section').replace('for_', '') ? 'active' : '';
                    $tabs.append('<li id="for_' + id + '" class="' + active + '">' + icon + ' <span data-hover="' + txt + '">' + txt + '</span></li>');
                    if (isLS && sessionStorage.getItem('mn-section') === 'for_mn_feedback') insertFeedbackForm($('#mn_feedback'))
                });

                // Process Appearance tab sub tabs
                $('#mn_appearance').prepend('<ul id ="mn_appearance_tabs" class="section-subtabs"></ul>');
                var $subtabs = $('#mn_appearance #mn_appearance_tabs');
                var $appearance = $('#mn_appearance .settings-form-wrapper');
                $('h1', $appearance).each(function (i, el) {
                    ...
                    var active = isLS && sessionStorage.getItem('mn-appearance-section') && id === sessionStorage.getItem('mn-appearance-section') ? 'active' : '';
                ...
                $subtabs.find('li').on('click', function () {
                    ...
                    if (isLS) {
                        sessionStorage.setItem('mn-appearance-section', id);
                    }
                ...
                if (isLS) {
                    ...
                    current = sessionStorage.getItem('mn-section');
                    current_sub = sessionStorage.getItem('mn-appearance-section');
                    ...
                    $active_sub = current_sub ? $('#mn_appearance_tabs li#' + current) : $('#mn_appearance_tabs li:first');
                    
                    $('#wrapper_' + current_sub, $appearance).addClass('active');

                    current = sessionStorage.getItem('mn-section-scroll');
                ...
                $tabs.find('li').not('#save-tab').on('click', function () {
                    ...
                    var $content = $('#mn-options-wrap #' + id);

                    if (id === 'mn_feedback') insertFeedbackForm($content);
                    if(id == 'mn_appearance' && !sessionStorage.getItem('mn-appearance-section')){
                        $('#mn_appearance_tabs #general').trigger('click');
                    }
                    ...
                    if (isLS) {
                        sessionStorage.setItem('mn-section', $t.attr('id'));
                    }
                ...
                if (isLS) {
                    $(window).unload(function (e) {
                        sessionStorage.setItem('mn-section-scroll', $('body').scrollTop() || $('html').scrollTop());
                    });

                    $wrap.find(':submit').click(function () {
                        sessionStorage.setItem('mn-section-submit', $(this).attr('id'));
                    });

                    if (sessionStorage.getItem('mn-section-submit')) {
                        //$saved = $('<div id="saved"> Saved!</div>');
                        $sbmt = $('#' + sessionStorage.getItem('mn-section-submit'));
                        ...
                        sessionStorage.setItem('mn-section-submit', '');
            ...
            document.getElementById('mn-options-wrap').style.opacity = 1;
        }
    }())
</script>
